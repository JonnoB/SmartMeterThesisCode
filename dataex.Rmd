...
cleandat <-smartdata[,c(1,(1+meterhc$order[lowerclustID]))]
size <- ncol(cleandat)*nrow(cleandat)
cleandat <- cleandat[lowertimepercs>0.9, c(TRUE,lowermeterpercs >0.99)]

ncol(cleandat)*nrow(cleandat)/size #amount of remaingin data

saveRDS(cleandat, "cleandata.rds")
...
```{r}
fulldays <- cleandat %>% group_by(date(Date.Time)) %>% summarise(total = n()) %>%
...
```{r}
meantest <- test %>% mutate_each(funs(rollmean(.,3, align="right")))

#find day half hour median for filling NA's
timemeds <- cleandat %>% mutate(day= day(Date.Time), min = minute(Date.Time), hour = hour(Date.Time)) %>%
  group_by(day, hour,min) %>%summarise_each(funs(mean(.,na.rm=TRUE))) %>% ungroup
...
datdat<-readRDS(list.files()[100]) %>%ungroup

# 
# meantest <- datdat[,-1] %>% lapply(., function(n) 
#   (rollmean(n,3, align="right"))%>% as.data.frame)
# 
# meantest <- bind_cols(meantest)
# names(meantest)<- names(datdat)[-1]


corlist <- datdat[,2:ncol(datdat)] %>% as.matrix %>% bigcor(., nblocks= 3, verbose = TRUE, method = "spearman")


# corlist <- meantest[,1:ncol(meantest)] %>% as.matrix %>% bigcor(., nblocks= 6, verbose = TRUE, method = "pearson")
...
cutoff <- data.frame(cutoff = seq(0.1,1,0.01), edges = NA,nodes=NA)

cutoff[,-1] <- lapply(cutoff$cutoff ,function(n) {
    logicmat<-distmat>n  
...
graph2 <-createcleangraph(graph, graph>0.7, 10)
graph2 <-detectcomms(graph2)
setwd(SubDataSets)
write.graph(graph2, "test3.graphml", format = "graphml")
...
daydata2<- daydata %>% group_by(ClusterID) %>% mutate(count=1) %>%
  summarise_each(funs(sum)) %>% ungroup%>% 
    gather(.,key=time, value=kwh, -ClusterID, -count) %>%
  mutate(time= ymd_hms(sub("X","",time)))

setwd(Figures)
daydata2 %>% filter(count>50) %>% ggplot(., aes(x=time, y=kwh, colour= ClusterID)) + geom_line() + 
  ggtitle("Sum Cluster Profiles")
ggsave("singledaypatternSum.pdf")

daydata3<- daydata %>%
  group_by(ClusterID) %>% mutate(count=n()) %>%
  summarise_each(funs(mean)) %>% ungroup%>% 
    gather(.,key=time, value=kwh, -ClusterID, -count) %>%
  mutate(time= ymd_hms(sub("X","",time)))


daydata3%>% filter(count>50) %>% ggplot(., aes(x=time, y=kwh, colour= ClusterID)) + 
  geom_line() + 
...
daydata4%>% filter(count>50) %>% ggplot(., aes(x=time, y=kwh, colour= ClusterID)) + 
  geom_line() + 
...
Take the nodes that ake up each vector and construct a vector that includes all nodes and ones where a node is included in the cluster
```{r}
...
for (i in 39:length(files)){
  ...
  setwd(file.path(basewd,"behaviourgraphs70"))
...
bigheat(jaccardclust, order="both", 
...
nodeclust4<-nodeclust2[,-1] %>%
  mutate_each(funs((./sum(.))^2))%>%t %>%as.data.frame %>%
  mutate(rowsums = rowSums(.)) %>%
  select(rowsums)%>%mutate(rownames = row.names(.))
...
test2 <-Clustconversion %>% select(-oldClustID)%>%
...
orderedheat(t(test2[,-1]), order="column", 
...
ClustCounts <-Clustconversion %>% group_by(ClustID) %>% summarise(counts = n(), first=first(day))
...
ClustCounts %<>% mutate(ClustIDSoup= ifelse(Large,ClustID,"Soup"))
ClustCounts$ClustIDSoup

match(ClustCounts$ClustIDSoup, nodeclust2$ClustID)
issoup <- ClustCounts$ClustIDSoup[match(nodeclust2$ClustID, ClustCounts$ClustID)]
...
Nodestab%>% filter(rowsums<0.6, rowsums>0.4) %>%  ggplot(., aes(x=rowsums)) +geom_density() +
  ggtitle("Density plot of node Stability") + xlab("sum of squared probability")
setwd(file.path(Figures,"Method"))
...
ClusTransSoup<- ClusterTransition %>% as.data.frame %>% mutate(ClustID=unlist(ClustCounts$ClustIDSoup)) %>%
  group_by(ClustID) %>%
  summarise_each(funs(sum)) %>% ungroup
rownames(ClusTransSoup)<- ClusTransSoup$ClustID
ClusTransSoup <- ClusTransSoup %>% select(-ClustID) %>% t %>%
  as.data.frame %>% mutate(ClustID=unlist(ClustCounts$ClustIDSoup)) %>%
  group_by(ClustID) %>%
  summarise_each(funs(sum)) %>% ungroup
rownames(ClusTransSoup)<- ClusTransSoup$ClustID
ClusTransSoup <- ClusTransSoup %>% select(-ClustID) %>% t
...
```{r}
setwd(datafile)
Customadata<-read.csv("CustomerTestCellDefinition.csv")

table(Customadata$Tariff.Type)
table(Customadata$Mo)
