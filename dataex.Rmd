...
daytimeseries <-file.path(basewd,"CleandataNAs")
...
x <-daydata %>% 
  ...
  ggplot(x, aes(x=kwh, colour=hour)) +geom_density()+
   ...
   x %>%
     ggplot(., aes(x=hour,y=kwh, group=noderef)) +geom_line(alpha=0.05)+
     facet_grid(ClusterID~.)+ggtitle("Exploration of the load profiles in each cluster")
   
   eucdist <-dist(daydata) %>%as.matrix()
   sum(is.na(eucdist))
   eucdist2 <-dist(daydata^2)
   diag(eucdist)<-max(eucdist)
    eucdist<-as.vector(eucdist)
   eucdist<-eucdist[(eucdist<1)]
   dens<-density(eucdist)
plot(dens)
