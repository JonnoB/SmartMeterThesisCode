...
```{r Packages}
packages <-c("stringr", "lubridate", "data.table","caret", "xgboost","e1071", "R.utils", "corrplot", "Hmisc", "Amelia", "Matrix", "ff", "ggdendro", "zoo", "igraph","parallel","dplyr", "magrittr", "ggplot2", "tidyr", "xtable")
...
GraphPath<-file.path("/home/rstudio", "graphs")
  ...
  basewd <- file.path(getwd(), "Thesis-Data")
...
rm(list= c("NACols","dayhourmin", "i","RowsFromMeanVals", "navect", "meanvals", "colID", "rowIDs"))
...
setwd(daytimeseries)
filename <- list.files()[101]#day 100 is the cool ring graph when cut off is .92
...
test <- lapply(nodeclustlist, function(n) n[,1]) %>% bind_cols() %>% t %>% as.data.frame %>% mutate_all(funs(as.factor))
...
test <- clusterprofile[[100]] %>% gather(., key= clusterID, value=kwh, -time) 
...
test<- Clusttrans %>% as.data.frame %>% filter(ClustID =="X9")

test[,colSums(select(test,-ClustID))>0]

Clustconversion %>% filter(ClustID=="X9")
Clustconversion %>% filter(UniqueID=="X9")
#Clustconversion %>% filter(oldClustID=="X9") #super long
(NodeClust[,-1] %>% colSums)[c(4293,6066)] #clearly not clusters x9

((NodeClust[,-1] %>% colSums)==357) %>% sum

rowSums(Clusttrans[,-1])

colSums(Clusttrans[,-1])

test180 <-nodeclustlist[[180]] #correct for 180
colSums(test180[,-1], na.rm=T)

test2011_10_28 <- nodeclustlist$`2011-10-28`  #this is the correct day for day 180
colSums(test2011_10_28[,-1], na.rm=T)

which(names(nodeclustlist)=="2011-10-28")
nodeclustlist[180] %>%names

test <- NodeClust[,-1] %>%colSums() 
which(test==314)
```
...
ClusTransSoup<- Clusttrans %>% as.data.frame %>% mutate(ClustID=unlist(ClusterID$ClustIDSoup)) %>%
  group_by(ClustID) %>%
  summarise_each(funs(sum)) %>% ungroup
