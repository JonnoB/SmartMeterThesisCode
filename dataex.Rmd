...
```{r savedaydata}
datevect <- cleandat$Date.Time %>%as.Date() 

setwd(daytimeseries)
for (i in 1:length(datevect)){
  filter(cleandat, as.Date(Date.Time)==datevect[i]) %>%
...
match(ClusterID$ClustIDSoup, nodeclust2$ClustID)
...
test <-ClusterDay %>%   left_join(., 
  ...
  test2 <- (select(test[-1,-1], -ClustIDSoup)>0)*1#select(test, -ClustIDSoup) #(select(test[-1,], -ClustIDSoup)>0)*1
  orderedheat((test2), order="none", merge=1) +
    ggtitle("Cluster Presence with Cluster\n and time Ordered by Similarity")+
    xlab("Time") +ylab("Cluster")

test3 <-createorder(test2)
