...
#why have some graphs not clustered?
test2 <- mclapply(files, function(n){
  print(n)
  graph <- read_graph(n, format="graphml")
  graph %>% get.vertex.attribute(., "ClusterID") %>%unique %>%length
  }, mc.cores= detectCores()
  ) %>% unlist
...
test2==test
setwd(Cormats)
list.files()[test==5260] %>% 
CreateAllGraphs(Cormats, GraphPath, parallel.mode = FALSE, files=.)

setwd(Cormats)
graph <-readRDS(list.files()[100])
sum(is.na(graph))
graph[is.na(graph)] <- 0
graph2 <-createcleangraph2(graph, graph>0.7 )
graph2 <-detectcomms(graph2)
...
clusterprofile<-mclapply(dates, function(n){
...
#creates a loadprofile data frame of all the uniqely named clusters across atime periods
clusterprofile <- lapply(1:length(clusterprofile), function(n){
    dayID <-n
    print(n)
    x <-clusterprofile[[dayID]]
...
test<- Clusttrans %>% as.data.frame %>% filter(ClustID =="X11")
...
```{r XGBoostmodel}
NorXGtre <- xgboost(as.matrix(select(NDM, -Mosaic.Class,-MosaicNumber)), as.factor(NDM$MosaicNumber), 
                    max.depth=5, 
                    eta=0.10,
                    nround = 100, 
...
XGpreds <- predict(NorXGtre, as.matrix(as.matrix(select(NDM, -Mosaic.Class,-MosaicNumber)))) 
...
test <- NodeClust2 %>% 
  ...
  select(-NodeID)
Mosaic<-test$MosaicNumber
names(test)<-make.names(names(test))
test <- hashed.model.matrix(~.,data=select(test, -MosaicNumber))

XGdayClass <- xgboost(test, as.factor(Mosaic), 
                    max.depth=5, 
                    eta=0.10,
                    nround = 100, 
                    objective = "multi:softmax",
...
XGpreds <- predict(XGdayClass, test) 

XGconf <- confusionMatrix(XGpreds, as.factor(Mosaic))

#best performers 5 7 10 13

test <-Customerdata %>% group_by(MosaicNumber) %>%
  summarise(Moasic.Class=first(Mosaic.Class))
