...
#Plot clusters across time, with clusteres ordered by similarity
  ...
  orderedheat(t(ClusterDay[,-1]), order="column", 
...
clusterprofile<-lapply(dates[1:10], function(n){
  ...
  daydata2 <- daydata %>% filter(., !is.na(ClusterID)) %>%
    ...
    mutate(time= format(x$time, format="%H:%M:%S"))
  })
...
orderedheat(test, merge=1, mid=0, legend = "corellation") + ggtitle("Cluster Corellation using Average Load Profile") +xlab("Cluster") +ylab("Clusters")
setwd(file.path(Figures,"Method"))
ggsave("ClusterCorLoad.pdf")

graph <- createcleangraph2(ClustComms, ClustComms >0.7)
      print("Graph created")
...
Clustconversion2 <- Clustconversion[1:213] %>% 
  mutate(ClustID=
           get.vertex.attribute(graph, "ClusterID")
)
