...
excludeSyncDropbox("/home/rstudio/Dropbox/Apps/ShareLaTeX/*")

includeSyncDropbox("/home/rstudio/Dropbox/Apps/ShareLaTeX/University-College-London-thesis/Figures")

includeSyncDropbox("Apps/ShareLaTeX")
excludeSyncDropbox("~/Dropbox/Apps/ShareLaTeX/*")
...
setwd(daytimeseries)
setwd(Cormats)
filename <- list.files()[177]#day 100 is the cool ring graph when cut off is .92

graph <- readRDS(filename)
graph2 <-createcleangraph2(graph, graph>0.7 )
...
#there appears to be 5258 nodes in every day when sweep is used for graph creation. When sweep is off then there are 5260
summary(nodecount)
```
...
#Fills in the spread variables with 0 this is an expected result and not a bug
...
#changeing nodeclustlist titles to unique names

lapply(nodeclustlist, function(n){
  x <- nodeclustlist[[n]] 
})
...
as.character(dates) %in% as.character(unique(Clustconversion$date))
...
allprofiles<-mclapply(dates, function(n){
  ...
  variable <- "mean"
  print(n)
  #the filtering of NA needs to be removed at some point as it will cause problems later on
  daydata2 <- daydata %>% filter(., !is.na(ClusterID)) %>%
...
data.frame(allprofiles= sapply(allprofiles, ncol),
           clustconv = clustdaycount$counts)

test <- allprofiles[[180]]
is.na(test) %>% sum
# 
# THe 177 missing row has been corrected in the fill in missing chunk
# #day 177 has a missing row, this is a hack to fill in the missing row with NAs

#Left join with day 1 as that will forces the data frame to open up and have a row of Na's at the missing time period
clusterprofile[[which(sapply(clusterprofile, nrow)==11)]]<- clusterprofile[[1]] %>%select(time) %>% left_join(.,clusterprofile[[which(sapply(clusterprofile, nrow)==11)]])

#fill the NA's with the mean of the values on either side
clusterprofile[[which(sapply(clusterprofile, function(n) sum(is.na(n))>0))]][6,-1] <- clusterprofile[[which(sapply(clusterprofile, function(n) sum(is.na(n))>0))]][c(5,7),-1] %>%t %>% rowMeans

#bind the columns together as they are all in time order
clusterprofile <- lapply(clusterprofile, function(n) n[,-1]) %>% bind_cols()


#thus function caused ordering problems on nodeclustlist so I am replacing it with my code wich solved the problem
# %>% Reduce(function(...) merge(..., by='time', 
#                              all.x=TRUE), .)

#Not needed now new code is completed
# #Fill in NA's caused by the missing half hour period in day 177/ 2011-10-25
# #Fill is done by finding the mean of the kwh on either side of the na
# clusterprofile[6,c(FALSE,is.na(colSums(clusterprofile[,-1])))] <- clusterprofile[c(5,7),c(FALSE,is.na(colSums(clusterprofile[,-1])))] %>%t %>%rowMeans
...
#not necessary after filling in the missing half hour in the lcusterprofile
...
setwd(file.path(Figures,"Method"))
ggsave("ClusterCorLoadall.pdf")

LargeClusts <- filter(Clustconversion, NodesInCluster>100)$UniqueID %>%
  match(.,colnames(ClustComms))


ClustComms2 <- ClustComms[LargeClusts,LargeClusts]

orderedheat(ClustComms2, merge=1, mid=0, simMat=TRUE, legend = "correlation") + ggtitle("Cluster Corellation using Average Load Profile") +xlab("Cluster") +ylab("Clusters")
ggsave("ClusterCorLoadLarge.pdf")

ClustComms2[is.na(ClustComms2)]<-0
Clustergraph <- createcleangraph2(ClustComms2, ClustComms2 >0.9)
      print("Graph created")
...
Why are there NA's appearing in day 177?
Not necessary now the root cause has been found
```{r discovering NA results}

sum(is.na(clusterprofile)) # Where do these NA's come from?

NAvect <- sapply(1:5808, function(n)sum(is.na(clusterprofile[,n])))
table(NAvect)
NAvect <- sapply(1:12, function(n)sum(is.na(clusterprofile[n,])))
clusterprofile[,c(TRUE,is.na(colSums(clusterprofile[,-1])))]

Clustconversion%>%filter(UniqueID=="X3951")
Clustconversion%>%filter(UniqueID=="X3971")

setwd(Cormats)
day177 <- readRDS(list.files()[177])
is.na(day177) %>% sum
setwd(daytimeseries)
day177time <- readRDS(list.files()[177])
is.na(day177time) %>% sum


datdat<-readRDS(list.files()[177]) %>%ungroup
      ...
      test <-sapply(datdat[,-1], function(n){
      var(n)==0
      }
      )
...
test <- orderedheat(is.na(weightmat)*1, simMat = TRUE, xblocks = 2, yblocks = 2, mid = 0.1)

(is.na(weightmat)*1) %>%sum

# Conclusion
# Sometimes a smart meter will return the same value as a constant for the whole period, this could be that no one was using the electricity, or that the value was a constant, either way these NA values need to be replaced with 0 that way there will be no corellation with any nodes and they will be free floating sub graphs.

#The above doesn't necessarily makes sense as the createcleangraph2 function converts all NA's to zero effectively performing the above. However sweep then removes the floating nodes

#This means that the NA's must simply be coming because day 177 has 11 time periods... which is obvious as when it is combined with the other time periods it is forced to have and extra space, and this space is then NA. The solution is to fill
```
...
UniqueClusters<- data.frame(UniqueID= filter(Clustconversion, NodesInCluster>100)$UniqueID, 
...
ggplot(nodeclust4, aes(x=rowsums)) +geom_density( fill="steelblue", alpha =0.7) +
...
```{r Cluster Plots}
Clustconversion %>% filter(NodesInCluster>100, IntraDayRank<=5) %>%
...
#Plot clusters across time, with clusteres ordered by similarity
  ...
  orderedheat(t(Clustsperday[,-1]), order="column", 
...
# ClusterTransition<- matrix(data=0, nrow= totalclusts,
#                            ncol=totalclusts,
#                            dimnames = list(unique(Clustconversion$ClustID),                                           unique(Clustconversion$ClustID)))
# 
...
NodeVolume <- sum(NodeClust[,-1])
percVolume <- rowSums(Clusttrans[,-1])/NodeVolume
...
nas <-filter(Clustconversion,is.na(issoup)) 
unique(nas$ClustID) 
 
match(Clustconversion$ClustID,ClusterID$ClustID)


issoup <- ClusterID$ClustIDSoup[match(Clustconversion$ClustID,ClusterID$ClustID)]
Clustconversion$ClustID <- issoup 
...
test <-Clustsperday %>%   left_join(., 
  ...
  orderedheat(t(test2), order="both", merge=1) +
...
test <- Clustconversion %>%group_by(day, date) %>%
summarise(counts=n()) %>% 
mutate(daynum = weekdays(day)) %>%
filter(day>95, day<110)
...
test <-SankeyData(95:102,SankeyFrame, Clustconversion, simple = F)
...
Clusttrans<- Clusttrans %>% as.data.frame %>% mutate(ClustID=unlist(ClusterID$ClustIDSoup)) %>%
  group_by(ClustID) %>%
  summarise_each(funs(sum)) %>% ungroup
rownames(Clusttrans)<- Clusttrans$ClustID
Clusttrans <- Clusttrans %>% select(-ClustID) %>% t %>%
...
```{r Clustertransgraph}
clustrans2 <- Clusttrans/rowSums(Clusttrans) #Clusttrans[-1,-1]
...
x <-daydata %>% 
  filter(ClusterID=="X13"|ClusterID=="X22"|ClusterID=="X6")%>%
...
NorXGtre <- xgboost(as.matrix(select(NDM, -Mosaic.Class,-MosaicNumber)),
                    NDM$MosaicNumber, 
                    max.depth=7, 
                    eta=0.1,
                    nround = 100, 
...
XGpreds <- predict(NorXGtre, as.matrix(select(NDM, -Mosaic.Class,-MosaicNumber))) 
...
set.seed(192)
idx <-  createDataPartition(make.names(NDM$MosaicNumber), p=0.7, list=FALSE)
#NDM$MosaicNumber <-NDM$MosaicNumber %>%make.names

trainx <-select(NDM[idx,], -Mosaic.Class,-MosaicNumber) %>% as.matrix
testx <-select(NDM[-idx,], -Mosaic.Class,-MosaicNumber) %>% as.matrix

trainy <-NDM$MosaicNumber[idx]
testy <-NDM$MosaicNumber[-idx]

NorXGtre <- train(x=trainx, y=make.names(trainy), method="treebag")

NorXGtre <- xgboost(trainx,
                    trainy, 
                    max.depth=3, 
                    eta=0.01,
                    nround = 500, 
...
XGpreds <- predict(NorXGtre, testx) 
XGpreds<- as.factor(XGpreds)

levels(XGpreds) <- levels(NDM$MosaicNumber)

XGconf <- confusionMatrix(XGpreds, testy)
XGconf

confusionMatrix(predict(NorXGtre, trainx), trainy)
...
#NA's are appearing from nowehere as well why?
#is.na(NodeClust2) %>%sum

#temporary measure replace NA with soup
#NodeClust2[is.na(NodeClust2) ]<-"soup"
