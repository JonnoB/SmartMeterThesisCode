...
data.frame(allprofiles= sapply(allprofiles, ncol)-1,
...
for (i in 1:length(clusterprofile)) {
  print(i)
  clusterprofile <- left_join(clusterprofile, clusterprofile[[i]], by="time")   
}
...
#remove clusters that have less than 1% of node volumn

LargeClusts <- Clustconversion %>%
...
SOmething is not going right somewhere, I hope I can fix it.
THere is som problem with the clusters not matching up again
```{r WTF Clusters}
...
length(days);length(Cors);length(Graphs)

Gstrip <-Graphs %>% sub("date_", "",.) %>% sub(".graphml", "",.)
Corstrip <- Cors %>% sub("date_", "",.) %>% sub(".rds", "",.)

#I found that I had made an additional graph and this was de-synching, have to check to see if this has fixed the error
