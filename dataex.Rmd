...
```{r Packages}
packages <-c("stringr", "lubridate", "data.table","caret", "xgboost","e1071", "R.utils", "corrplot", "Hmisc", "Amelia", "Matrix", "ff", "ggdendro", "zoo", "networkD3", "igraph","parallel", "magrittr", "ggplot2", "tidyr", "xtable","entropy","dplyr", "microbenchmark")
...
#Missing days--- The below code findds which days are missing from clusterconversion
  alldays <-seq(from=min(clustest$date) , to=max(clustest$date),by = "day") 
alldays[!(alldays %in% unique(Clustconversion$date))] 
...
IntCorVis <-orderedheat(IntCorList[[1]], order="both", simMat = TRUE, merge = 1, mid = 0)
IntCorVis

IntCorVis2 <-orderedheat(abs(IntCorList[[1]]), order="both", simMat = TRUE, merge = 1)
IntCorVis2

  test <- sapply(1:50, function(n) mean(abs(IntCorList[[n]]))) %>% data.frame(value=., NodeID= NodeIDs)  
  
  ggplot(test, aes(x= value)) + geom_density(fill="steelblue", alpha =0.7) +
    labs(title= "Within node absolute correllation across", 
         x= "Mean absolute Corellation" )


  cleandata[,2] %>%filter(hours(Date.Time) == 1600)
  
  
  test <- IntCor %>% filter(NodeID ==1010) 

  ggplot(test,aes(x = Date, y=`21:00`)) +geom_point()
    ggplot(test,aes(x=`16:0`)) +geom_density()

    cor(test$`18:30`[-243],test$`18:30`[-1])

test2 <-    test %>%select(`18:30`, Date)
   
acf(test2$`18:30`) 

acf(rep(1,100)+rnorm(100,0,0.1))
...
test2 %>% 
  filter(ClustID=="1600"|ClustID=="1830"|ClustID=="2100") %>%
     ...
     scale_color_manual(values= ggplotColours(10)[c(1,4,8)], guide=FALSE)
...
ggplot(NodeEntropy, aes(x=entropy)) +geom_density( fill="steelblue", alpha =0.7) +
  ggtitle("Density plot of Entropy") + xlab("Entropy")+geom_vline(aes(xintercept=entropy(rep(0.1,10)), colour="red"))+theme(legend.position="none") 
...
PredTrans <- PredTrans %>% as.data.frame %>% bind_cols(select(Clustconversion, ClustID, day),.)%>% filter(day<=200)%>% select(-day) %>% group_by(ClustID) %>%summarise_each(funs(sum)) %>% ungroup %>% select(-ClustID)


PredTrans <- PredTrans %>% t %>% 
  as.data.frame %>% 
 bind_cols(select(Clustconversion, ClustID, day),.)%>% filter(day<=200)%>% select(-day)%>% 
  group_by(ClustID) %>%
  summarise_each(funs(sum)) %>% ungroup 
names(PredTrans)[-1] <- PredTrans$ClustID

PredTrans <- PredTrans[,-1]
...
#Uses days 200 to 242 to predict 201 to 243
predictmat <- lapply(200:242, function(n){

source <-Clustconversion %>% filter(day ==n) %>% group_by(ClustID)%>%
...
targetmat <- lapply(201:243, function(n){
...
# MSE <-(predicted-target$NodesInCluster) %*%  t(predicted-target$NodesInCluster) %>% sqrt

data.frame(NodeRMSE= (predictmat-targetmat)^2 %>% rowSums %>% sqrt)

#Create load profile for each cluster on each day
DayProfile <-clusterprofile %>% 
  gather(., key=UniqueID, value= kwh, -time) %>%
  left_join(., 
            select(Clustconversion, day, ClustID, UniqueID), 
            by = "UniqueID") %>% select(-UniqueID) %>% 
  group_by(day, ClustID, time) %>% 
  summarise(kwh=mean(kwh)) %>%
  spread(key= time, value= kwh)


#vector of days 201 to 243
datevect <- (Clustconversion %>% filter(day>200))$date %>% unique

#The actual mean profile for the the nodes in the test set
DayProfile <- DayProfilemat(datevect, AllClusts)

#Join the predicted node amounts to the date the prediction is for and gather the resulting data frame
predictmat2 <- bind_cols(data.frame(date=datevect), predictmat) %>% 
  gather(., key = ClustID, value = Nodes,-date)


targetmat2 <- bind_cols(data.frame(date=datevect), targetmat) %>% gather(., key=ClustID, value= Nodes, -date)
...
PredLoad<-DayProfile%>%
  mutate_at(.,vars(matches("0")),funs(.*lag(predictmat2$Nodes,10)))%>% select(-ClustID)%>%group_by(date) %>% 
...
resids <- (DayProfile2[-1,-1]-PredLoad[-1,-1]) %>% as.data.frame %>%
  bind_cols(select(PredLoad[-1,],date),.) %>% mutate(day = weekdays(date))


#dates have to be shifted 1 day as the prediction is one day ahead
colourframe <- data.frame(date=datevect) %>% mutate(colour = ifelse(date=="2011-12-24","#FF0000","#000000"),colour = ifelse(date=="2011-12-23","#33ff00", colour))
...
ResNoX <- results %>% filter(date!="2011-12-24",date!="2011-12-25" ) 

#not much point in plotting this 
 ResNoX%>% ggplot(., aes(x=time,y= PercErr, 
       group = date)) +
  geom_line(alpha=0.5) +  
  theme(legend.position="none") +
...
ResNoX %>%select(-time) %>% #group_by(date) %>%
...
moddat <- Portfolio %>% select(kwh,daylag, weeklag, date) %>% filter(!is.na(weeklag))

train <-moddat %>% filter(date <="2011-11-18")

test <-moddat %>% filter(date >"2011-12-18")

mod1 <- train(x= train[,2:3], y= train[,1], method="lm")

linpreds <- predict(mod1, test[,2:3])

res <- postResample(linpreds, test[,1] )
res
mape(linpreds, test[,1] )

#check bayesian model
postResample(results$Predicted, test[,1])
mape(results$Predicted,test[,1])
...
Portfolio %>% group_by(time, weekday) %>% summarise(kwh= mean(kwh), sd = sd(kwh)) %>% mutate(group=1, lsd= kwh-sd(kwh), usd=kwh+sd(kwh))%>% 
  ggplot(., aes(x=time, y= kwh, group= weekday)) +
