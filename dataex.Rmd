...
#Create a matrix of the target number of Node in each cluster per day
...
test <- bind_cols(data.frame(date=datevect), predictmat) %>% 
  gather(., key = ClustID, value = Nodes,-date)

# get the profile of each cluster
test2 <-DayProfile%>%
  mutate_at(.,vars(matches("0")),funs(.*test$Nodes))

test3 <-DayProfile%>%
  mutate_at(.,vars(matches("0")),funs(.*lag(test$Nodes,10)))%>% select(-ClustID)%>%group_by(date) %>% 
...
resids <- (DayProfile2[-1,-1]-test3[-1,-1]) %>% as.data.frame %>%
  bind_cols(select(test3[-1,],date),.) %>% mutate(day = weekdays(date))

x <- resids[,c(1,14, 2:13)]

resids^2 %>%rowMeans() %>% sqrt
...
results <-left_join(
  gather(DayProfile2, key= time, value=Actual, -date),
              gather(test3, key= time, value=Predicted, -date),
          by=c("date", "time")) %>%
